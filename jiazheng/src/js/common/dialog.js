;(function($,window,QNR){
		...
		var _isIE  = !!($.browser.msie),
		    _isIE6 = (_isIE && $.browser.version === '6.0'),
			$doc   = $(window.document),
			$body  = $('body'),
			$win   = $(window); 
        ...
        //背景 前景 
		var dlg_mask_html = '<div class="dlg_mask"></div>',
			dlg_box_html = '<div class="g_dlg_box"></div>';

		var dlgid = "dlg",mids=ids = 0,
			_d_zindex = 100000;
		...
		QNR.popMsg = function(msg){
			var dia  = new Dialog({
                ...
                newMask : true,
				content : [
					'<div class="pop_msg" style="position:relative;" ><table><tr><td>',
					msg,
					'</td></tr></table><i class="js_close" style="position:absolute;width:16px;height:20px;right:5px;top:5px;"></i></div>'
				].join(''),
				closeBtn : {
					handler : function(dlg){
					   if(timer){
